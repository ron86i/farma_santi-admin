...
import { Button, Input, Select, SelectContent, SelectGroup, SelectItem, SelectLabel, SelectTrigger, SelectValue } from "@/components/ui";
import { useForm } from "react-hook-form";
import { ClienteId, ClienteRequest, TipoCliente } from "@/models";
import { useRegistrarCliente } from "@/hooks/useCliente";
import { useClientesContext } from "@/context";
import { CustomToast } from "@/components/toast";
import dateFormat from "dateformat";
import { toast } from "sonner";

// Schema con enum TipoCliente
// Schema con enum
...
interface ModalRegistrarClienteProps {
  ...
  onConfirmed?: () => void;
  open: boolean;
  onEnviarDato?: (valor: ClienteId) => void;
}

export function ModalRegistrarCliente({ open, onClose, onConfirmed,onEnviarDato }: ModalRegistrarClienteProps) {
  const { mutate } = useRegistrarCliente();
  ...
  // Detecta cambios en tipo
  const tipoValue = form.watch("tipo");

  // Limpia complemento si tipo es NIT
  useEffect(() => {
  ...
  const onSubmit = async (data: ClienteFormData) => {
    ...
    try {
      const response = await mutate(clienteData);
      toast.custom((t) => (
        <CustomToast
          t={t}
          type="success"
    ...
    if (onEnviarDato) {
      onEnviarDato(response?.data as ClienteId); // aquí se envía al padre
    }
