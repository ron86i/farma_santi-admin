...
import { useAnularOrdenCompra, useQuery } from "@/hooks";
...
interface ModalAnularOrdenCompraProps {
    compraId: number;
    onClose?: () => void;
    open: boolean;
}

export function ModalAnularOrdenCompra({ compraId, open, onClose }: ModalAnularOrdenCompraProps) {
    const { data: dataCompra, fetch: fetchCompra, loading } = useQuery(obtenerCompraById);
    const { mutate: anularOrdenCompra } = useAnularOrdenCompra()
    ...
    const onSubmit = async () => {
        try {
            const response = await anularOrdenCompra(compraId)
            toast.custom((t) => (
                <CustomToast
                    t={t}
                    type="success"
            ...
            toast.custom((t) => (
                <CustomToast
                    t={t}
                    type="error"
                    title="Error al anular orden de compra"
            ...
            >
                <DialogHeader className="space-y-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <DialogTitle className="text-xl font-semibold">Anulación de Compra</DialogTitle>
                            <DialogDescription>
