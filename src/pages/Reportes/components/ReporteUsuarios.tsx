...
export function ReporteUsuarios() {
  const navigate = useNavigate();
  const titulo = "Reporte de Usuarios";
  const descripcion = "Visualiza la lista completa de usuarios registrados.";
  ...
  const construirURL = () => {
    ...
    return `${baseUrl}/reportes/usuarios?${query.toString()}`;
  };
  ...
  const descargarPDF = () => {
    ...
    link.href = pdfUrl;
    link.download = `reporte-usuarios-${new Date().toISOString().split("T")[0]}.pdf`;
