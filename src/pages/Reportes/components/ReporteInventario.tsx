...
import { obtenerListaLaboratorios, obtenerListaFormasFarmaceuticas } from "@/services";

export function ReporteInventario() {
  const navigate = useNavigate();
  const titulo = "Reporte de Inventario";
  const descripcion = "Stock disponible en almacén.";

  // Traer laboratorios y formas farmacéuticas
  const { data: laboratorios, fetch: fetchLaboratorios } = useQuery(obtenerListaLaboratorios);
  const { data: formas, fetch: fetchFormas } = useQuery(obtenerListaFormasFarmaceuticas);

  // Filtros
  const [laboratorioId, setLaboratorioId] = useState("");
  const [formaFarmaceuticaId, setFormaFarmaceuticaId] = useState("");
  ...
  const construirURL = () => {
    ...
    if (laboratorioId && laboratorioId !== "Todos") query.append("laboratorioId", laboratorioId);
    if (formaFarmaceuticaId && formaFarmaceuticaId !== "Todos") query.append("formaFarmaceuticaId", formaFarmaceuticaId);
    if (estado && estado !== "Todos") query.append("estado", estado);
    return `${baseUrl}/reportes/inventario?${query.toString()}`;
  };
      ...
      <div className="bg-white dark:bg-neutral-900 border rounded-lg p-4 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              value={formaFarmaceuticaId}
              onValueChange={setFormaFarmaceuticaId}
              disabled={loading || !formas}
            >
              <SelectTrigger className="w-full">
